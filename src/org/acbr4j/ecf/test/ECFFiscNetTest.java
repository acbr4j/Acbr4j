...
public class ECFFiscNetTest extends TestCase {
	
	private static final String PORTA = "COM3";

	public ECFFiscNetTest( String name ) {

		super( name );
	}

	public void testGenerico() {
		//ECFFiscNet ecf = new ECFFiscNet( PORTA );
	...
	public void testComandosDeCupomFiscal() {
		...
		ECFFiscNet ecf = new ECFFiscNet("COM3", 1700);
		...
		for(int i=0;i<40;i++){
			...
			assertTrue( trataresultFuncao( ecf.vendaItem( 
					"000001", "Produto Teste", "-2", 'I', 1f, 10f, 'D', 0f ) ) );
		}
...
//		assertTrue( trataresultFuncao( ecf.iniciaFechamentoCupom( ECFElginX5.DESCONTO_PERC, 20.0f ) ) );
		...
		String indice = ecf.programaFormaPagamento( "Dinheiro" );
		assertTrue( trataresultFuncao( ecf.efetuaFormaPagamento( indice, 510.00f, "" ) ) );
